...
function afc_validate_fonts( $input ){
	...
	if( !file_exists( ADVANCEDFONTCHANGERDIR . 'fonts/local' ) )
		$message .= __( 'AFC fonts folder not exists, please make sure: plugins-directory/advanced-font-changer/fonts/local folder exists', 'afc_textdomain' );
		$type = 'error';
	...
	if( $message == '' ){
		foreach( $formats as $format ){
			...
			$info = pathinfo( $thisfile );
			$fileaddress = $uploaddir['path'] . '/' . $info['basename'];
			rename( $uploaddir['path'] . '/' . $info['basename'], ADVANCEDFONTCHANGERDIR . 'fonts/local/' . $format. '/' . $info['basename'] );
